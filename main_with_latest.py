...
class MfapiLatestNAVResponse(BaseModel):
    meta: MfapiSchemeMeta
    data: List[MfapiNAVData]
    status: Literal["SUCCESS", "ERROR"]
...
async def fetch_scheme_latest(client: httpx.AsyncClient, code: int) -> MfapiLatestNAVResponse:
    data_raw = await mfapi_get_json(client, f"{MFAPI_BASE_URL}/{code}/latest")
    data = MfapiLatestNAVResponse.model_validate(data_raw)
    if data.status != "SUCCESS":
        raise RuntimeError(f"mfapi latest returned {data.status} for scheme {code}")
    return data


def extract_latest_nav_point(nav_items: List[MfapiNAVData]) -> Optional[Tuple[date, Decimal]]:
    if not nav_items:
        return None
...
async def process_scheme(session: AsyncSession, client: httpx.AsyncClient, code: int, job_id: int) -> None:
    ...
    latest_response: Optional[MfapiLatestNAVResponse] = None
    latest_target_key: Optional[Tuple[str, str]] = None
    latest_point: Optional[Tuple[date, Decimal]] = None

    try:
        latest_response = await fetch_scheme_latest(client, code)
        latest_target_key = verified_target_key(latest_response.meta)
        if latest_target_key is None:
            raise RuntimeError(f"scheme {code} latest response does not match target AMC/category/direct-growth filters")
        latest_point = extract_latest_nav_point(latest_response.data)
        if latest_point is None:
            raise RuntimeError(f"scheme {code} latest response contained no NAV data")

        latest_nav_date, latest_nav_value = latest_point
        if not fund:
            fund = Fund(code=code)
            session.add(fund)

        fund.name = latest_response.meta.scheme_name
        fund.amc = latest_response.meta.fund_house or latest_target_key[0]
        fund.category = latest_target_key[1]
        fund.scheme_type = latest_response.meta.scheme_type
        fund.isin_growth = latest_response.meta.isin_growth
        fund.isin_div_reinvestment = latest_response.meta.isin_div_reinvestment
        ...
        logger.exception("Latest NAV shortcut failed for scheme %s; falling back to full history", code)
    ...
    latest_point = extract_latest_nav_point(data.data)
